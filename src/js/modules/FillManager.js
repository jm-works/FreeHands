export class FillManager {
    ...
    fill(startX, startY, hexColor, tolerance = 32, opacity = 1, clipBounds = null, onComplete = null) {
        ...
        if (clipBounds) {
            ...
            if (startX < cl || startX >= cr || startY < ct || startY >= cb) {
                if (onComplete) onComplete();
                return;
            }
        }
        ...
        (async () => {
            ...
            const layerId = layerManager ? layerManager.activeLayerId : null;
            ...
            const prevURL = await historyManager.captureLayerDataURL(layerId);
            const prevObjects = historyManager.snapshotLayerObjects(layerId);
            ...
            if (this.colorMatch(startR, startG, startB, startA, targetColor)) {
                if (onComplete) onComplete();
                return;
            }
            ...
            fabric.Image.fromURL(tempCanvas.toDataURL(), async (fillImg) => {
                fillImg.set({
                    left: 0,
                    top: 0,
                    selectable: false,
                ...
                this.canvas.add(fillImg);
                this.canvas.requestRenderAll();

                const nextURL = await historyManager.captureLayerDataURL(layerId);
                historyManager.rasterCommand(prevURL, nextURL, layerId, prevObjects);
