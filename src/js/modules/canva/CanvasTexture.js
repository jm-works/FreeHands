export class CanvasTexture {
    constructor(cm) {
        this.cm = cm;
        this.currentTextureId = 'none';
        this.textureCache = {};
    }

    setPaperTexture(textureId, silent = false) {
        this.currentTextureId = textureId;

        const width = this.cm.canvas.width;
        const height = this.cm.canvas.height;
        const cacheKey = `${textureId}_${width}x${height}`;

        let dataURL;

        if (silent && this.textureCache[cacheKey]) {
            dataURL = this.textureCache[cacheKey];
        } else {
            ...
            this.textureCache[cacheKey] = dataURL;
        }
        ...
        fabric.Image.fromURL(dataURL, (img) => {
            ...
            if (!silent && this.cm.historyManager) {
    ...
    reapplyTexture() {
        if (this.currentTextureId) {
            this.setPaperTexture(this.currentTextureId, true);
        }
    }
}